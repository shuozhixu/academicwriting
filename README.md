...
- Sometimes two BibTex entries with different citation keys may point to the same reference. This usually happens when two entries were obtained in different ways, e.g., one directly from the journal website, another from Zotero. Check all references in your `.bib` file to remove duplicates.
