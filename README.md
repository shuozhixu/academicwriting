...
- There are two main citation styles: numberic and author-year:
	- The author-year style will take care of the authors' names automatically, regardless of how many authors there are. But for the numerical style, you need to explicitly write the authors' names, if needed. Use their last names only. When there is one author, e.g., King, write `King [34] has a dream`. When there are two authors, e.g., Lewis and Clark, write `Lewis and Clark returned to St. Louis on September 23, 1806`. When there are more than two authors, e.g., Weiss, Barish, and Thorne, write `Weiss et al. won the 2017 Nobel Prize in Physics`.
