...
- Make sure that the [special character(s)](https://en.wikibooks.org/wiki/LaTeX/Special_Characters), if any, are used in authors' names.
- Make sure that the greek letters appear correctly, e.g., `$\alpha$ iron` instead of `a iron`; another example is it should be `{$\Sigma 3$} grain boundary` instead of `S 3 grain boundary` or `$\sigma 3$ grain boundary` which would yield "$\sigma 3$ grain boundary".
